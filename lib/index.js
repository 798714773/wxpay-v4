...
const decryptByApiV3 = function(
    apiV3Key,
    associate, // 加密参数 - 类型aad
    nonce, // 加密参数 - 随机数 iv
    ciphertext, // 加密密文
){
...
class Payment {
    ...
    async decodeCertificates() {
        ...
        for (let cert of certificates) {
            cert.decrypt_certificate = this.decode(cert.encrypt_certificate)
    ...
    decodeResource(resource) {
        return JSON.parse(this.decode(resource));
    }

    //解密
    decode(params) {
        let { ciphertext, associated_data, nonce } = params;
        return decryptByApiV3(this.apiv3_private_key, associated_data, nonce, ciphertext)
    }
